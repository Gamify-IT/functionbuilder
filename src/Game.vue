...
export default defineComponent({
  ...
  setup() {
    ...
    if (selectedMode.value === 'Algebra') logicBlocks.value = algebraBlocks;
    else if (selectedMode.value === 'Lambda') logicBlocks.value = lambdaBlocks;
    else if (selectedMode.value === 'Strings') logicBlocks.value = stringBlocks;

    const players = ref([
      { id: 1, name: playerName.value, blocks: [], result: null, input: selectedMode.value === 'Strings' ? 'hello' : 2 },
      { id: 2, name: 'Player 2', blocks: [], result: null, input: selectedMode.value === 'Strings' ? 'world' : 4 },
    ]);

    const targetOutput = ref(selectedMode.value === 'Strings' ? 'HELLO!' : 16);
    ...
    const dragStart = (block) => { draggedBlock = block; };
    const dropBlock = (id) => {
    ...
    const showHint = () => {
      ...
      if (player) hints.value = player.blocks.map((block) => block.hint);
    };

    const hideHint = () => { showingHint.value = false; };

    onMounted(() => {
      socket.emit('joinRoom', { roomId: gameId.value, playerId: playerName.value });
      setInterval(() => { socket.emit('requestPlayerData', gameId.value); }, 5000);
      socket.on('updatePlayerData', (data) => { console.log('Player data:', data); });
    });

    onUnmounted(() => { socket.disconnect(); });

    return { logicBlocks, players, targetOutput, dragStart, dropBlock, executeChain, selectedMode, currentLevel, movesLeft, maxBlocks, showingHint, hints, showHint, hideHint };
  },
});
</script>
